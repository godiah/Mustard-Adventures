...
@section('title', 'Create Adventure Type - Mustard Adventures')
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="mb-8">
            <div class="flex items-center mb-4">
                ...
                <h1 class="text-4xl font-heading font-bold text-secondary">Create New Adventure</h1>
            </div>
            <p class="text-gray-600 font-body">Add a new adventure to your collection</p>
        </div>

        <!-- Form -->
        <div class="max-w-5xl mx-auto bg-white rounded-lg shadow-md p-8">
            <form method="POST" action="{{ route('adventures.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select id="days" name="days"
                        ...
                        @foreach (App\Models\Adventure::getDaysOptions() as $value => $label)
                            <option value="{{ $value }}" {{ old('days') == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <select id="people" name="people"
                        ...
                        @foreach (App\Models\Adventure::getPeopleOptions() as $value => $label)
                            <option value="{{ $value }}" {{ old('people') == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <!-- Image Upload -->
                <div class="mb-6">
                    <label for="image" class="block text-sm font-medium text-secondary mb-2">Adventure Image</label>
